...
def save_chunks_to_files(chunks, output_dir, min_char_length=20):
    """
    将分割后的文本块保存到文件。如果文本块字符数少于min_char_length，则合并到前一个文件中。

    参数:
    chunks (list): 文本块列表
    output_dir (str): 输出目录路径
    min_char_length (int): 每个文件的最小字符数，默认为20
    """
    # 如果输出目录不存在，则创建
    ...
    for i, chunk in enumerate(chunks):
        # 如果当前块字符数少于min_char_length，则合并到前一个块
        if len(chunk) < min_char_length:
            previous_chunk += chunk
        else:
            # 如果之前有内容积累，则将其与当前块合并保存
            if previous_chunk:
    ...
    # 如果最后一个块累积的字符数少于min_char_length，将其保存
    if previous_chunk:
        chunk_file_path = os.path.join(output_dir, f"chunk_{len(chunks)}.txt")
        with open(chunk_file_path, 'w', encoding='utf-8') as file:
            file.write(previous_chunk)
        print(f"已保存最后一个累积的文本块到 {chunk_file_path}")


# 主程序

# 设置输入和输出路径
input_file_path = 'D:/git/gitstorege/data/input/txt/xingnuxunlianxueyuan.txt'  # 替换为你的长文本文件路径
output_dir = 'D:/git/gitstorege/data/output//txt/pixiv/xingnuxunlianxueyuan/'  # 替换为你希望保存文本块的目录路径
...
similarity_threshold = 0.7  # 可根据需要调整
